...
-- PARTITION BY aircraft_model: Это определяет окно, в пределах которого будет выполняться нумерация.
-- Мы хотим, чтобы нумерация была выполнена для каждой уникальной модели самолета.
-- Таким образом, каждая различная модель самолета будет иметь свое собственное окно для нумерации.
-- ORDER BY flight_count DESC: Это определяет порядок сортировки для нумерации.
-- Мы сортируем аэропорты назначения в порядке убывания количества вылетов (flight_count).
-- Это означает, что самый частый аэропорт будет иметь наивысший flight_count, именно этот аэропорт получит ранг 1.
